...
static cv::Scalar obj_id_to_color(int obj_id) {
    //                                Y         B             R
    double const colors[6][3] = { { 0,1,1 },{ 1,0.5,0 },{ 0,0.5,1 },{ 1,0,1 },{ 0,0,1 },{ 0,1,0 } };
...
class preview_boxes_t {
    enum { frames_history = 1 };    // how long to keep the history saved
            ...
            for (auto &k : result_vec) {
                if (preview_box_track_id[i].track_id == k.track_id) {
                    ...
                    if (rect_roi.width > 1 || rect_roi.height > 1) {
                        ...
                        cv::resize(roi, dst, cv::Size(preview_box_size, preview_box_size), cv::INTER_LINEAR);
